...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ctx.logger.info(`[cortex-plugin-jira] Loaded`);
    ...
    try {
      ...
      if (!project_key || typeof project_key !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'project_key must be a non-empty string',
      ...
      if (args.labels && typeof args.labels === 'string') {
        fields.labels = args.labels.split(',').map((l: string) => l.trim()).filter(Boolean);
      }
      ...
      if (!res.ok) {
        return {
          ...
          success: false,
          output: '',
          error: `Jira API error ${res.status}: ${JSON.stringify(data)}`,
          durationMs: Date.now() - start,
        };
      }
      return {
    ...
    try {
      ...
      if (!jql || typeof jql !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'jql must be a non-empty string',
          durationMs: Date.now() - start,
        };
      }
    ...
    try {
      ...
      if (!updates || typeof updates !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'updates must be a non-empty JSON string',
          durationMs: Date.now() - start,
        };
      }
      ...
      if (res.status === 204) {
        return {
          toolName: 'jira_update_issue',
          success: true,
          output: `Issue ${issue_key} updated successfully`,
          durationMs: Date.now() - start,
        };
      }
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (typeof board_id !== 'number') {
        return {
          toolName: 'jira_sprint_report',
          success: false,
          output: '',
          error: 'board_id must be a number',
          durationMs: Date.now() - start,
        };
      }
      if (typeof sprint_id !== 'number') {
        return {
          ...
          success: false,
          output: '',
          error: 'sprint_id must be a number',
          durationMs: Date.now() - start,
        };
      }
      const base = (config.jiraUrl || 'https://your-domain.atlassian.net').replace(/\/$/, '');
      ...
      const res = await fetch(
        `${base}/rest/greenhopper/1.0/rapid/charts/sprintreport?rapidViewId=${board_id}&sprintId=${sprint_id}`,
        {
      ...
      if (!res.ok) {
        return {
          ...
          success: false,
          output: '',
          error: `Jira Agile API error ${res.status}: ${JSON.stringify(data)}`,
          durationMs: Date.now() - start,
        };
      }
      return {
